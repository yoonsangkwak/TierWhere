...
class HistorySummonerService(val view: HistorySummonerView) {
    ...
    fun tryGetSummoner(summonerName: String) {
        val historySummonerRetrofitInterface = ApplicationClass.sRetrofit.create(HistorySummonerRetrofitInterface::class.java)
        historySummonerRetrofitInterface.getSummoner(summonerName).enqueue(object : Callback<Summoner> {
